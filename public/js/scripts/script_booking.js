...
for(let i=0; i<block.length;i++){
    let open = block[i].querySelector('#book');
    ...
    open.addEventListener('click', function(e){
        e.preventDefault();
        block[i].nextElementSibling.classList.add('active'); })

    let close = block[i].nextElementSibling.querySelector('#popup__close');
    close.addEventListener('click', () => {
        block[i].nextElementSibling.classList.remove('active'); })
